...
@ClientGameTest
public class BabyAgeUnlockTest implements ClientTest {
  @Override
  public void runTest(ClientTestContext context) {
    try (ClientWorld world = context.worldBuilder().survival().create()) {
      var stat = Stats.CUSTOM.get(MoreStats.BABY_AGE_UNLOCKED);
      int before = world.getStat(stat);

      world.summon(
          EntityType.PIG,
          world.playerBlockPos().south(),
          "{Age:-24000,AgeLocked:1b,NoAI:1b,PersistenceRequired:1b,Silent:1b,Tags:[\"msbabyageunlock\"]}"
      );
